...
export default function CreatePayment() {
    ...
    getDocs(
      ...
      .then((snapshot1) => {
        ...
        snapshot1.forEach((document) => {
          getDocs(
            query(
              collection(firestore, "users", document.data().id, "accounts"),
              where("iban", "==", iban)
            )
          )
            .then((snapshot2) => {
              if (snapshot2.empty) {
                ...
                debitSnap.then((debitDoc) => {
                  if (Number(debitDoc.data().balance) < Number(amount)) {
                    alert(
                      "Der Betrag ist zu hoch als das Ihr Konto zur Verfügung hat."
                    );
                  } else {
                    updateDoc(debitRef, {
                    ...
                    creditSnap.then((creditDoc) => {
                      updateDoc(creditRef, {
                        balance:
                          Number(creditDoc.data().balance) + Number(amount),
                      });
                    });
                    createTransaction(
                      document.data().id,
  ...
  const createTransaction = (creditorId, receiverSurname, receiverName) => {
    getDoc(doc(firestore, "users", auth.currentUser.uid)).then((userSnap) => {
      createTransferDoc(
        auth.currentUser.uid,
        creditorId,
        receiverSurname,
  ...
  return (
    <section className="bg-gray-50 dark:bg-gray-900">
      <Navbar />
      <div className="p-6 space-y-4 md:space-y-6 sm:p-8 bg-base-100 flex justify-center">
        <div className="w-2/3 max-[800px]">
          <h1 className="text-xl font-bold leading-tight tracking-tight text-gray-900 md:text-2xl dark:text-white pb-10">
            Zahlung erfassen
          </h1>
          <form className="space-y-4 md:space-y-6" onSubmit={preTransfer}>
            ...
            <button
              type="submit"
              className="w-full text-white bg-primary-600 hover:bg-primary-700 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-emerald-600 dark:hover:bg-emerald-700 dark:focus:ring-emerald-800"
            >
              Ausführen
            </button>
